...
namespace kBackup.Classes
{
    class Requests
    {
        ...
        /// <summary>
        /// Retrieves Json formatted data from URL.
        /// </summary>
        /// <param name="url">Url to retrieve Json formatted data from.</param>
        /// <returns>Json formatted data as String.</returns>
        public static async Task<string> GetJsonAsync(string url)
        {
            var client = new HttpClient();
            try
            {
                client.DefaultRequestHeaders.Authorization = AuthenticationHeaderValue.Parse(GetAuthHeader(Settings.Default.email, Settings.Default.password));
                using (var response = await client.GetAsync(url))
                {
                    ...
                    switch (response.StatusCode)
                    {
                        ...
                        case HttpStatusCode.NotFound:
                            ErrorOccurred = true;
                            ErrorMessage = @"The Zendesk domain you are requesting does not exist.";
                            break;

                        case HttpStatusCode.InternalServerError:
                            ErrorOccurred = true;
                            ErrorMessage = @"There appears to be an issue with the login server, please try again later.";
                            break;

                        case (HttpStatusCode)429:
                            ErrorOccurred = true;
                            ErrorMessage = @"You have hit the rate limit.";
            ...
            {
                MessageBox.Show(Form.ActiveForm, @"The general exception was hit. If you continue to receive this message please contact the developer at felix.stubner@gmail.com with the following message: " + e.Message);
            }
        ...
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.userApi, Settings.Default.domain));
            try
            {
                var user = JsonConvert.DeserializeObject<ZApiModel.UserCollection>(jsonData);
                ...
                var newJsonData = JsonConvert.SerializeObject(user);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\user\\profile.json", _fileSystem.AES_Encrypt(newJsonData, Settings.Default.encryptPassword));
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetCategoryData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.categoryApi + "?per_page=100", Settings.Default.domain));
            try
            {
                //var categories = JsonConvert.DeserializeObject<ZApiModel.CategoryCollection>(jsonData);

                //var newJsonData = JsonConvert.SerializeObject(categories);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\Categories.json", jsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetSectionData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.sectionApi + "?per_page=100", Settings.Default.domain));
            try
            {
                var sections = JsonConvert.DeserializeObject<ZApiModel.SectionCollection>(jsonData);

                var newJsonData = JsonConvert.SerializeObject(sections);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\Sections.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetArticleData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.articleApi + "?per_page=100", Settings.Default.domain));
            try
            {
                var articles = JsonConvert.DeserializeObject<ZApiModel.ArticleCollection>(jsonData);

                var newJsonData = JsonConvert.SerializeObject(articles);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\Articles.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetArticleCommentData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.userApi, Settings.Default.domain));
            try
            {
                ...
                var newJsonData = JsonConvert.SerializeObject(user);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\ArticleComments.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetTopicData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.userApi, Settings.Default.domain));
            try
            {
                ...
                var newJsonData = JsonConvert.SerializeObject(user);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\Topics.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetPostData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.userApi, Settings.Default.domain));
            try
            {
                ...
                var newJsonData = JsonConvert.SerializeObject(user);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\Posts.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
        ...
        public async Task<bool> GetPostCommentData()
        {
            var jsonData = await GetJsonAsync(string.Format(Settings.Default.userApi, Settings.Default.domain));
            try
            {
                ...
                var newJsonData = JsonConvert.SerializeObject(user);
                File.WriteAllText(Settings.Default.dataFolder + "\\kbackup\\data\\json\\PostComments.json", newJsonData);
            }
            catch (Exception)
            {
                return false;
